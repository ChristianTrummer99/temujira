...
export const ROUTES = {
  ...
  // ---- managed agents + reservations (exclusive agent assignments) ----
  "managedAgents.list": {
    method: "GET",
    path: "/managed-agents",
    auth: "user",
    scope: "reservations:manage",
    summary: "List admitted agent identities with availability and current reservation",
    query: ListManagedAgentsQuerySchema,
    response: listOf(ManagedAgentSchema),
  },
  "managedAgents.admit": {
    method: "POST",
    path: "/managed-agents",
    auth: "user",
    scope: "reservations:manage",
    summary: "Admit an existing agent account to the reservation workflow",
    body: AdmitManagedAgentInputSchema,
    response: z.object({ agent: ManagedAgentSchema }),
  },
  "managedAgents.remove": {
    method: "DELETE",
    path: "/managed-agents/:userId",
    auth: "user",
    scope: "reservations:manage",
    summary: "Un-admit an agent identity (requires no active reservation)",
    response: okResponse,
  },
  "reservations.list": {
    method: "GET",
    ...
    auth: "user",
    scope: "reservations:manage",
    summary: "List reservations (filter by agent, task, status or request id)",
    query: ListReservationsQuerySchema,
    response: listOf(ReservationSchema),
  },
  "reservations.lookup": {
    method: "GET",
    ...
    auth: "user",
    scope: "reservations:manage",
    summary: "Recover a committed claim by request id without creating new ownership",
    query: LookupReservationQuerySchema,
    response: z.object({ reservation: ReservationSchema.nullable() }),
  },
  "reservations.current": {
    method: "GET",
    path: "/reservations/current",
    auth: "user",
    summary: "Identify the caller's reservation (worker credential self-check)",
    response: z.object({
      user: UserSchema,
  ...
  "reservations.get": {
    ...
    auth: "user",
    scope: "reservations:manage",
    summary: "Inspect one reservation (identity, ticket, run reference, key id, lifecycle)",
    response: z.object({ reservation: ReservationSchema }),
  },
  "reservations.claim": {
    method: "POST",
    path: "/reservations",
    auth: "user",
    scope: "reservations:manage",
    summary: "Atomically reserve an agent identity + ticket and mint the worker key (token once)",
    body: CreateReservationInputSchema,
    response: z.object({
      reservation: ReservationSchema,
      /** Null on an idempotent replay — the token is only ever revealed by the first claim. */
  ...
  "reservations.release": {
    ...
    auth: "user",
    scope: "reservations:manage",
    summary: "Release a reservation: revoke its key and clear the matching assignment",
    body: ReleaseReservationInputSchema,
    response: z.object({ reservation: ReservationSchema }),
  },
