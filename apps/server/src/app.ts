...
async function readJsonBody(request: Request): Promise<unknown> {
  ...
  while (true) {
    ...
    if (size > JSON_BODY_LIMIT) {
      await reader.cancel();
      throw new HttpError("payload_too_large", "request body too large");
    }
    chunks.push(value);
  }
...
async function seedAdminFromEnv(ctx: AppContext): Promise<void> {
  ...
  console.log(
    `[temujira] created initial admin ${adminEmail} from environment`,
  );
}
...
export async function buildApp(config: ServerConfig): Promise<BuiltApp> {
  ...
  const mountOrder = [...ROUTE_IDS].sort(
    (a, b) => paramCount(ROUTES[a].path) - paramCount(ROUTES[b].path),
  );
